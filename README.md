...
1) Submissions must be written by the submitter and licensed under MIT or BSD-3.
...
2) Submissions must compile with both Rust stable 1.69 and 1.84.
...
8) Submissions may build a cache and store it in a static at runtime.

The time to build the cache will be counted as part of the time for the
benchmark to execute however.

9) Implementations must run in constant time.
